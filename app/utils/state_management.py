...
def initialize_session_state():
    ...
    IMPORTANT: Only initializes if not already present to prevent reruns.
    """
    ...
    # Initialize AI chat system flag (lazy loading - only when needed)
    if 'ai_chat_initialized' not in st.session_state:
        st.session_state.ai_chat_initialized = False
        st.session_state.ai_chat = None
        st.session_state.ai_key_status = "not_initialized"
...
def initialize_ai_chat():
    """
    Lazy initialization of AI chat - only call this when AI chat is actually needed.
    This prevents unnecessary initialization on every page load.
    """
    if st.session_state.ai_chat_initialized:
    ...
    st.session_state.ai_chat_initialized = True
